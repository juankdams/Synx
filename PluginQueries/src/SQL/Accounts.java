...
public class Accounts{
	//Queries
	private final String insertAccount = "INSERT INTO `accounts` ( `ID` , `username` , `password` , `pseudo` , `secretquestion` , `secretanswer` , `gmlvl`, `community`, `curIP`, `isBanned`, `premiumTime`, `persos`, `banks`, `friends`, `ennemies`, `ignored`)"+// `spellboost` , `capital` , `energy` , `size` , `gfx` , `account`,`cell`,`map`,`spells`,`objets`,`storeObjets`)" +
	" VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";//,?,?,?,?,?,?,?,?,?,'', '');";
	private final String updateAccountQuery = "UPDATE `accounts` SET `gmlvl`= ?, `curIP`= ?, `isBanned`= ?, `premiumTime`= ?, `giftID`= ?, `persos`= ?, `banks`= ?, `friends`= ?,`ennemies`= ?,`ignored`= ? WHERE `ID` = ?;";
	
	private final String updatePersosQuery = "UPDATE `accounts` SET `persos`= ? WHERE `accounts`.`ID` = ?;";
	private final String updateRelationsQuery = "UPDATE `accounts` SET `friends`= ?,`ennemies`= ?,`ignored`= ? WHERE `ID` = ?;";
	private final String updateGiftQuery = "UPDATE `accounts` SET `giftID`= ? WHERE `accounts`.`ID` = ?;";
	private final String updateBanQuery = "UPDATE `accounts` SET `isBanned`= ? WHERE `accounts`.`ID` = ?;";
	//private static String loadAccountsQuery = "SELECT * from accounts WHERE `username` = 'accname';";
	
	
	//Methods
	public LoginObjects.LAccount load(LoginServer s, String username){
		try {
	...
	public void updatePersos(LoginObjects.LAccount account) {
		if(account == null){
	 ...
	 */
	public void updateRelations(LoginObjects.LAccount account) {
	...
	public void updateGift(LAccount a, int giftID){
		try {
	 ...
	 * 0 = correct, 1 = banni
	 */
	public void updateBan(LAccount a, boolean banned){
		try {
	 ...
	 * @return
	 */
	public String findAccountPersoList(LoginServer s, String pseudo) {
		try {
	 ...
	 */
	public void create(LAccount account) {
		try {
	 ...
	 */
	public int loadLastAccountID(LWorld s) {
		ResultSet RS = null;
		int ID = 0;
		try {
