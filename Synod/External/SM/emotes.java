...
public class emotes extends Sender{
	 ...
	 */
	public static void eU(AbstractCharacter perso, int emoteID, int timer){
		for(ISprite z : perso.getMap().getSpritesSynchroFull()){
			if(z.getCharacterType() == CharacterType.PLAYER && z.getLayer() == perso.getLayer()){
				if(timer > 0){
	 ...
	 */
	public static void eD(GlobalObjects.Map map, AbstractCharacter p, byte dir)
	{
		...
		for(ISprite z : map.getSpritesSynchroFull()){
			if(z.getCharacterType() == CharacterType.PLAYER && z.getLayer() == p.getLayer()){
				send(((GPersonnage)z).getClient().getSession(),"eD"+p.getID()+"|"+dir);
			}
		}
	}
	/*
