...
public class CollectionQuery<T> implements Iterable<T> {
	...
	public T best(Comparator<T> comparator) {
		T best = null;
		for(T t : this){
			if(best == null){
				best = t;
			}
			if(comparator.compare(t, best) == 1){
				best = t;
			}
		}
		return best;
	...
	/* //Fail
	 * public CollectionQuery<T> orderAllBy(Comparator<T> comparator) {
		T best = null;
		CollectionQuery<T> copy = new CollectionQuery(null);
		for(int i = count()-1; i >= 0; i--){
			for(T t : from){
				if(best == null){
					best = t;
				}
				if(comparator.compare(best, t) == 1){
					best = t;
				}
			}
			copy.from(best);
	...
	public ArrayList<T> computeList() {
