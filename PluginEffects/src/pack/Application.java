...
public class Application extends AbstractEffectPluginListener{
	...
	protected void apply(GEffect e, IEffectType et, AbstractCharacter caster, AbstractCharacter target, ParameterHolder ph, CombativeCharacter castercombat, CombativeCharacter targetcombat){
		...
		switch((SpellEffectType)et){
			...
			case PLUS_X_LIFE://110
				...
				Syn.d("life given in stats.incrementPdv(life) = "+life+"  operateur ="+et.getOperator());
				targetcombat.getStatistique().incrementPdv(life, targetcombat);
				break;
			case PLUS_X_PA://111
			case MINUS_X_PA://168
			case PLUS_X_PM://128
			case MINUS_X_PM://169
			case MINUS_X_RANGE://116
			case PLUS_X_RANGE://117
				//Stats
			case PLUS_X_FORCE:
			case MINUS_X_FORCE:
			case PLUS_X_CHANCE:
			case MINUS_X_CHANCE:
			case PLUS_X_INTELLIGENCE:
			case MINUS_X_INTELLIGENCE:
			case PLUS_X_AGILITE:
			case MINUS_X_AGILITE:
			case PLUS_X_SAGESSE:
			case MINUS_X_SAGESSE:
			case PLUS_X_VITALITE:
			case MINUS_X_VITALITE:
				short stat = (short) (e.getParams().isDice() ? e.getDice().getRandomShot() : e.getJet().getValue());
				if(et.isNegative()) stat *= -1;
				Syn.d("(EffectApplication:) "+et.getCharacType().name()+" given in BOOST column = "+stat+"  operateur ="+et.getOperator());
				targetcombat.getStatistique().incrementValue(et.getCharacType(), stat, CharacteristicColumn.BOOST);
				break;
			...
			case RETURN_X_DAMAGES://220
				...
				targetcombat.getStatistique().decrementPdv(e.getJet().getValue(), targetcombat);
			...
			case TELEPORT://825
				...
				Map m = caster.getWorld().getMap(ph.mapID);
			...
			case MAXIMISE_EFFET_ALEATOIRE://782
				...
				GBuff b = GBuff.provideBuff(caster.getWorld()).createNew(e, tours, BuffApplicationType.OneShot);
	...
	private void notice(IEffectType et, AbstractCharacter caster, AbstractCharacter target){
		if(target == null){
			SM.Messages.M(((GPersonnage)caster), "L'effet "+et.getID()+" "+et.name()+" n'avait aucun target sur qui être appliqué !", Constants.COLOR_PINK);
		}else
	...
	@Override
	public boolean isMountSheetEffect(GEffect e) {
		return e.getEffectID() == SpellEffectType.CONSULTER_FICHE_MONTURE.getID();
	}
	...
	public IEffectType getAddPdvMaxEffect() {
		return SpellEffectType.PLUS_X_VITALITE;
	}
	@Override
