...
if (!dofus.aks.Game)
{
    ...
    _loc1.onTurnMiddle = function (sExtraData)
    {
        ...
        while (++_loc5, _loc5 < _loc3.length)
        {
            ...
            if (_loc6.length != 0)
            {
                var _loc7 = _loc6[0];//spriteID
                var _loc8 = _loc6[1] == "1" ? (true) : (false);//hasSpriteOverHead & removeIt
                var _loc9 = Number(_loc6[2]);//pdv
                var _loc10 = Number(_loc6[3]);//PA
                var _loc11 = Number(_loc6[4]);//PM
                var _loc12 = Number(_loc6[5]);//orientation/direction
                var _loc13 = Number(_loc6[6]);//rien...
                var _loc14 = Number(_loc6[7]);//pdvMax
                _loc4[_loc7] = true;
    ...
    _loc1.onMovement = function (sExtraData, bIsSummoned)
    {
        ...
        while (++_loc5, _loc5 < _loc4.length)
        {
            ...
            if (_loc8)//Si le sprite est en addition OU modification (GM~ et GM-)
            {
				...
				/* |->  plusieurs choses:
						...
						templateID pour un pnj,
				
				*/
                var _loc16 = _loc10[5];//spriteType
                var _loc17 = _loc10[6];//gfx^size
                var _loc18 = false;//_loc27.noFlip = _loc18;
                if (_loc17.charAt(_loc17.length - 1) == "*")
                {//Donc si le loc17 (gfx^size) finit par un "*" comme ca : gfx^size*, on met lsprite en noflip
                    _loc17 = _loc17.substr(0, _loc17.length - 1);
                    _loc18 = true;
                } // end if
                var _loc19 = _loc17.split("^");//(gfx^size)
                var _loc20 = _loc19.length == 2 ? (_loc19[0]) : (_loc17);//gfxID
                var _loc21 = 100;//sizeX
                var _loc22 = 100;//sizeY
                if (_loc19.length == 2)//Set les sizeX et Y si elles sont spécifiées dans le (gfx^size) Pcq l,emu pourrait juste envoyer le gfx si on voulait
                {
                    var _loc23 = _loc19[1];
                    if (_global.isNaN(Number(_loc23)))
                    {//on peut déterminer sizeX et Y individuellement: (gfx ^ SizeX x SizeY)
                ...
                if (_loc7)//loc7 = Si le sprite est en modification (GM~)
                {
                ...
                switch (_loc16)
                {
                    ...
                    case "-3"://MonsterGroup
                    {
                        ...
                        var _loc30 = this.sliptGfxData(_loc17); //loc17 = gfx^size,gfx^size,gfx^size,gfx^size
                    ...
                    case "-9"://Dragodinde dans un enclos
                    {
    ...
    _loc1.onCellObject = function (sExtraData)
    {
        ...
        while (++_loc5, _loc5 < _loc4.length)
        {
            ...
            var _loc7 = Number(_loc6[0]);//cellID
            var _loc8 = _global.parseInt(_loc6[1]);//itemTemplateID
            if (_loc3)
            {
                var _loc9 = new dofus.datacenter.Item(0, _loc8);
                var _loc10 = Number(_loc6[2]);//0 ou 1 pour savoir si y'a de la durabilité ou pas 
                switch (_loc10)
                {
                    case 0:
                    {
