...
if (!ank.utils.Sequencer)
{
    ...
    _loc1.execute = function (bForced)
    {
		...
		{  //bForced = pour forcer à exécuter l'action même si le Séquencer est déjà en train d'en play une.
            return;
        } // end if
        this._bPlaying = true;
        var _loc3 = true;
        while (_loc3)
        {
            ...
            {
				var _loc4 = this._aActions[0]; //_loc4 = la première action à exécuter dans l'array
                if (_loc4.waitEnd)
                {
                ...
                if (!_loc4.waitEnd)
                {			//à ce moment, ça enleve l'action finie de dedans l'array 
							//et shift c'qui reste dans l'array pour passer à la prochaine action
