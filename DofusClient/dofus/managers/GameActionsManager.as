...
if (!dofus.managers.GameActionsManager)
{
    ...
    _loc1.transmittingMove = function (type, params)//Le type ici est TOUJOURS 1. (J'ai fait la recherche)
    {
        if (!this.isWaiting())//si yes pas en train d'attendre qu'une action soit finie (de mouvement sur map)
        {
            this.api.network.GameActions.sendActions(type, params);
            this._state = dofus.managers.GameActionsManager.STATE_TRANSMITTING;
            this._currentType = type;
        }
        else if (this.canCancel(type))//si l'action a le droit d'être cancellée (de mouvement sur map)
        {
            this.cancel(this._data.cellNum);//cancelle le mouvement en cours
            this.transmittingMove(type, params);//démarre le nouveau mouvement.
        }
        else
        {
