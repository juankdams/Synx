...
public class Packet2048SendCharacters implements OutgoingPacket {
	...
	public OutPacket encode() {
		...
		for (GameCharacter character : player.getCharacters()) {
			...
			writeID(character, buffer);
			writeIdentity(character, buffer);
			writeName(character, buffer);
			writeBreed(character, buffer);
			writeAppearance(character, buffer);
			writeEquipment(character, buffer);
			writeCreationData(character, buffer);
			writeXP(character, buffer);
			writeNation(character, buffer);
