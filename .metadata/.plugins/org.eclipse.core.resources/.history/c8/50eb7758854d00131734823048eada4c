...
public abstract class LoginServer implements IService {
	...
	public ArrayList<LoginClient> clients = new ArrayList<LoginClient>();
	...
	public LoginServer(short ID, String dbHostIP, String dbName, String dbUser, String dbPass, String ownerss) {
		this.ID = ID;
	...
	public void off(boolean terminate) {
		ArrayList<LoginClient> copy = new ArrayList<LoginClient>(clients.size());
		copy.addAll(clients);
		//synchronized(clients){
			for(LoginClient lc : copy){
				lc.kick();
			}
			clients.clear();
			get().stop();
