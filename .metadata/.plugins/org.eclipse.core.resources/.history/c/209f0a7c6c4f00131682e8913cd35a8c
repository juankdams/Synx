 ...
 *
 */
public class CrystalParser extends AbstractPacketParser<TClient, String> {
		...
		switch(id){
			...
			case listOwnedLoginServers:
				...
				for(Integer i : c.getUser().ownedLoginServers){
					...
					LoginServer lserv = Syn.Services.serversService.loginServers.get(i.shortValue());
					if(lserv == null){
						continue;
					}
					if(servers.length() > 0){
			...
			case bootLogin:
			{
				String loginid = packet.substring(4, packet.length());
				if(ownsLoginServer(c, loginid)){
					LoginServer s = Syn.Services.serversService.loginServers.get(Short.parseShort(loginid));
					try {
						s.boot();
