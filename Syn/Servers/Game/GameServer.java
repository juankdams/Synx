...
public abstract class GameServer implements IService, IObject {
	...
	public ArrayList<DofusGameClient> clients = null;
	...
	public GameServer(LoginServer s, GConfig config, CWorld cworld){
		super();
		this.loginServer = s;
	...
	public void boot() throws Exception {
		...
		clients = new ArrayList<DofusGameClient>();
		...
		scripts = new ScriptsService<ExecutableScript>(){
			...
			@Override public void boot() throws Exception {
				SQL.SConfig.loadScripts(world, null);
			}
		};
		scripts.boot();
		
		pluginsManager = new PluginsManager(getLoginServer().getGameClientStrategy().defaultPluginsManager){
			@Override public void loadPlugins() {
				SConfig.loadPlugins(world, null);
				SConfig.loadMacros(world, null);
			}
		};
		pluginsManager.boot();
		...
		pluginsManager.plugins.getPluginManager().callEvent(MiscEvent.Set_GameClientStrategy, world.getStrategy().get());
		...
		Syn.d("Finit de créer le GServer id("+this.getServerID()+") Vous pouvez maintenant le .on();");
	...
	public void off(boolean terminate){
		...
		ArrayList<DofusGameClient> copy = new ArrayList<DofusGameClient>(clients);
		//copy.addAll(clients);
		//Collections.copy(copy,clients);
		synchronized(clients){
			Syn.d("GServer.off 2!");
			for(DofusGameClient gc : copy){
	...
	public ArrayList<DofusGameClient> getClients() {
		return clients;
	}
	...
	public PluginsManager getPluginsManager(){
		return this.pluginsManager != null ? this.pluginsManager : 
			this.getLoginServer().getGameClientStrategy().defaultPluginsManager;
	}
	...
	public static GameServerProvider provide(GameClientStrategy gcs) {
		return gcs.defaultPluginsManager.getProviders().provideGameServer();
	}
