...
public class IASchedulePacker implements Runnable{
	 ...
	 * Object[1] = date (par currentMillis) à laquelle il faut envoyer les packets de larray
	 */
	...
	public static void schedulePackets(PrintWriter[] pws, String[] packet){
		...
		for(int i = packet.length; i-- > 0;){
			Syn.d("i = "+i);//TODO vérifie le i-- > 0; dans les for
	...
	public void run() {
		while(!t.isInterrupted()){
			...
			for(ArrayList<Object[]> al : pool){
				for(Object[] p : al){
					//Sleep le temps d'arriver à la bonne date
					if((Long)p[1] < System.currentTimeMillis()){
						try {
							...
							//->Pour lui v c'tun while pcq bon on attend là x_x
							//Thread.sleep(15);
						} catch (NumberFormatException e) {e.printStackTrace();
						} catch (InterruptedException e) {e.printStackTrace();}
					}
					//Envoie les packets à tous les destinataires
					for(int i = p.length-1; i-- > 2;){
						Syn.d("i = "+i);
						for(IoSession is : (IoSession[])p[0]){
							SM.Sender.send(is, p[i]+"");
						}
					}
				}
				al.clear();
