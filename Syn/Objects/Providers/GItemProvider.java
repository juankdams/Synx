...
public class GItemProvider implements IProvider {
	...
	public GItem createNew(int ID, IItemTemplate template, short quantity, boolean parfait, AbstractWorld w){
		if(template == null){
		...
		i.setItemEffects(template.getTemplateEffects(), parfait, w);
	 ...
	 */
	public GItem reloadInstance(int ID, IItemTemplate template, String totalEffects, short quantity, byte position, AbstractWorld w){
		GItem i = new GItem();
		if(template == null){
		 ...
		 */
		i.setItemEffects(totalEffects, true, w);
		i.setQuantity(quantity);
