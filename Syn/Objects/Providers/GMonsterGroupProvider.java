...
public class GMonsterGroupProvider implements IProvider {
	...
	public GMonsterGroup createNew(AbstractWorld w, short size){
	...
	public GMonsterGroup createNewInstance(ArrayList<GMonster> monsters, String[] drops, DofusGameServer s){
		GMonsterGroup mg = new GMonsterGroup();
	...
	public GMonsterGroup loadInstance(short groupID, String monstersIDs, String type, AbstractWorld w){
		GMonsterGroup mg = new GMonsterGroup();
		...
		if(monstersIDs != null && monstersIDs.length() > 0){
			for(String str : monstersIDs.split(",")){
				...
				GMonster m = w.getMonsterTemplate(templateid)[grade];
				mg.add( m );
			}
