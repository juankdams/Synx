...
//	private ArrayList<GDropItem> droppedItems = null;  [Intégrés dans l'ArrayList de sprites ;)]
	...
	public List<GFight> getFightsSynchroCopy() {//non modifiable
	    ...
	    synchronized(fights){
	    	ArrayList<GFight> fightscopy = new ArrayList<GFight>();
	    	fightscopy.addAll( fights ); //Collections.unmodifiableList( fights );
	    	return fightscopy;
	    }
	}
	/*
	...
	public synchronized ArrayList<GFight> updateFightsList(MapAction action, GFight f){
		if(action == ADD_FIGHT && f != null){
			...
			fights.add(f);
			SM.fights.fC_LAYER(this, f.getOldLayer());
	...
	public int getFightCount(byte layer){
		ArrayList<GFight> ofights = updateFightsList(MapAction.GET_FIGHTS, null);
		if(ofights == null){
			return 0;
		}
		ArrayList<GFight> fights = new ArrayList<GFight>(ofights);
		for(int i = 0; i <= fights.size()-1; i++){
			if(fights.get(i).getOldLayer() != layer){
				fights.remove(i);
			}
		}
		return fights.size();
		...
		if(sprites != null && sprites.size() > 0){
			for(ISprite s : sprites){
				if(s.getCharacterType() == CharacterType.MONSTER_GROUP){
					...
					GMonsterGroup copy = base.copy(gs);
	...
	public GSubArea getSubArea(AbstractWorld w) {
		return w.getSubArea(subArea);
	}
