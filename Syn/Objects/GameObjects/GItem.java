...
public class GItem implements IObject{
	...
	public static GItemProvider provide(AbstractWorld w){
		PluginsManager pm = w.getStrategy().defaultPluginsManager;
		if(w.getServer() != null){
			w.getServer().getPluginsManager();
		}
		return pm.getProviders().provideItem();
	}
	
	public static GItem copy(GWorld gw, GItem item){
		EffectsManager em = gw.getServer().getEffectsManager();
		return GItem.provide(gw).reloadInstance(gw.getNextItemID(), item.getTemplate(), item.getEffectsPacket(em), item.getQuantity(), item.getPosition(), gw);
	}
	 ...
	 */
	public void setItemEffects(String itemEffects, boolean parfait, AbstractWorld w){
		if(itemEffects == null){
			this.effects = null;
			return;
		}
		...
		for(String effet : effets){
			GEffect e = EffectsShare.getSimilarEffect(effet, parfait, w);
			if(e != null){
