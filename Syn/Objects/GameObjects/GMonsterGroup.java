...
public class GMonsterGroup extends AbstractCharacter{
	...
	public static GMonsterGroupProvider provide(AbstractWorld w){
		PluginsManager pm = w.getStrategy().defaultPluginsManager;
		if(w.getServer() != null){
			w.getServer().getPluginsManager();
		}
		return pm.getProviders().provideMonsterGroup();
	}
 ...
 */
	...
	public void terminate() throws Throwable{
		...
		if(type == HEROIC_INSTANCE){
			SQL.Monsters.saveInstance(this, getWorld());
		}
		for(GMonster m : monsters){
			m.terminate();
		}
	...
	public void setDrops(String[] drops) {
		if(drops != null && drops.length > 0 && drops[0].equals("") == false){
			...
			for(int i = drops.length-1; i >= 0; i--){// New .... Old
				specialDrops[i] = GDrop.provide(getWorld()).createNew(drops[i], true);
			}
		}else if(drops == null){
			specialDrops = null;
		}
	 ...
	 */
	public GMonsterGroup copy(GameServer newServ){
	...
	public void attachToMap(Map map, short cellid) {
