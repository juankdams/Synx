...
public class GPersonnage extends CombativeCharacter{
	...
	public static GPersonnageProvider provide(AbstractWorld w){
		PluginsManager pm = w.getStrategy().defaultPluginsManager;
		if(w.getServer() != null){
			w.getServer().getPluginsManager();
		}
		return pm.getProviders().providePerso();
	}
	...
	private DofusGameClient client = null;
	 ...
	 * OnJoinGame
	 */
	public void onSelected(DofusGameClient client){
		setClient(client);
	...
	*/
	...
	public void setClient(DofusGameClient client) {
		this.client = client;
	}

	public DofusGameClient getClient() {
		return client;
	}
