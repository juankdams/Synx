...
import parsers.dofus.AccountParser;
import parsers.dofus.BasicParser;
import parsers.dofus.ChatParser;
import parsers.dofus.ConquestParser;
import parsers.dofus.CrystalParser;
import parsers.dofus.DialogsParser;
import parsers.dofus.DocumentsParser;
import parsers.dofus.EnvironmentParser;
import parsers.dofus.ExchangesParser;
import parsers.dofus.FightsParser;
import parsers.dofus.GameParser;
import parsers.dofus.GuildsParser;
import parsers.dofus.HousesParser;
import parsers.dofus.InformationsParser;
import parsers.dofus.JobsParser;
import parsers.dofus.KeyParser;
import parsers.dofus.ObjectsParser;
import parsers.dofus.PartyParser;
import parsers.dofus.PingParser;
import parsers.dofus.QuestsParser;
import parsers.dofus.RelationsParser;
import parsers.dofus.RidesParser;
import parsers.dofus.SpellsParser;
import parsers.dofus.TutorialsParser;
import parsers.dofus.WayPointsParser;
import parsers.wakfu.LoginParser;
import parsers.wakfu.VersionParser;
...
public class ParsingMain extends Plugin implements PluginListener {
	...
	public void initiateWakfuParsers(){
		//Syn.d("initiateWakfuParsers");
		ParsingManager pam = pm.getAdapter().getParsingManager();
		pam.getParsers().clear();
		pam
		.setParser(1025, new LoginParser())
		.setParser(7, new VersionParser())
		;
	}
	...
	public void initiateActionListenersAlias(){
		if(strategy != GameClientStrategies.DofusClientStrategy) return; //[DEBUG] FIXME
