...
public class Objects {
	...
	public static void Ow(IoSession out, GPersonnage p){
		send(out, "Ow"+p.getInventory().getUsedPods()+'|'+(FormulingManager.calculateMaxPods(p.getStatistique(), p)));
	}
	 ...
	 */
	public static void OAKO(IoSession out, GItem... item) {
		...
		for(int i = item.length-1; i>=0; i--){
			...
			.append(item[i].getEffectsPacket());
		}
