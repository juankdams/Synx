...
public class Account{
		...
		StringBuilder str = new StringBuilder("Ag")
			...
			for(String item : c.getAccount().getGift().getItems()){
				...
				if(args.length == 2){//TODO packet de liste de cadeaux pas fini
					IItemTemplate it = GlobalWorld.getItemTemplate(c.getHandler().gServ.getWorld(), Integer.parseInt(args[0], 16));
					if(it != null){
	 ...
	 */
	public static void Ad_Ac_AH_AlK_AQ(LClient c, String pseudo, int level, String question, byte community)
	{//modified for SYN
		...
		for(GServer serv : Syn.gameServers){
			...
			packet.append(serv.getConfig().getWorldID()+";").append(serv.getState()).append(";").append(serv.getWorld().getcWorld().population).append(";").append(serv.getWorld().getcWorld().type);
		}
	...
	public static void AH(GClient c){
		String packet = "AH";
		for(GServer serv : Syn.gameServers){//Liste des serveurs
			if(packet.length() > 2){
				packet += "|";
			}
			CWorld cw = serv.getWorld().getcWorld();
	...
	{
		byte worldID = c.getHandler().gServ.getConfig().getWorldID();
		byte num = 0;
		StringBuilder packet = new StringBuilder();
	 ...
	 */
	public static void AxK(LClient c)
	{//modified for SYN
		...
		for(GServer gs : Syn.gameServers){
			...
			for(int i = number; i-- >0;){//Corrigé ici le 13dec12 à 1h11 du mat. c'était avant un if(number != 0)
				packet.append("|").append(gs.getConfig().getWorldID()).append(",").append(number);
			}
		}
		send(c.getSession(), packet.toString());
	}
	 ...
	 */
	public static void AXK_AYK(IoSession out,int accountID, GServer serv)
	{
		...
		*/
		...
		//modified for SYN
		send(out, "AYK"+serv.getConfig().getIP()+':'+serv.getConfig().getPort()+';'+accountID);
	 ...
	 * this.aks.Account.onSelectServer(!bError, true, sData.substr(3));
	 */
	public static void AXK(IoSession out,int accountID, GServer serv)
	{
		//modified for SYN
		send(out, new StringBuffer("AYK").append(serv.getConfig().getIP()).append(":").append(serv.getConfig().getPort()).append(";").append(accountID).toString());
	}
	...
	public static void As(IoSession session, GPersonnage p)
	{
		...
		.append(p.getStatistique().getPdvMax()).append('|')
		...
		.append(FormulingManager.calculateTotalInitiative(p.getStatistique(), p.getLevel())).append('|')
		.append(FormulingManager.calculateTotalProspection(p.getStatistique())).append('|')
		...
		p.getStatistique().getPacket(str);
