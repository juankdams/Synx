...
public class SpellApplication {
	...
	private void incrementCooldownEffects(GSpell s, CombativeCharacter caster, ArrayList<CombativeCharacter> targets){
		...
		//Ajout de l'effet de nombre de coup maximal par tour
		...
		//Ajout de l'effet de nombre de coup maximal par tour par target
				...
				for(ISprite t : sometargets){
					...
					if(caster.getBuffs() != null){
						for(GBuff b : caster.getBuffs()){
							...
							if(b.getEffectID() == pm.getEffectsManager().getMaxLaunchByTurnByTargetEffect().getID() &&
								b.getDice().getNumberOfFaces() == t.getID() &&//Si le buff vise bien le bon target
